...
async def oauth(code: str):
    ...
    - Home에서 Kakao Login 폼으로 url 이동
    - 로그인 진행(완료)
    - Kakao Developer 에 미리 설정 해 놓은 Redirect uri로 Authorization Code가 전달됨.
    - 설정해둔 Redirect uri가 본 함수인 /ouath 임.

    Authorization Code를 전달받은 본 API는 다음과 같은 과정을 수행.
    1. Authorization code를 다시 Kakao 서버에 요청을하여, Access / Refresh Token을 발급
    2. Access Token를 Kakao API에 전달하여, 사용자 정보 획득
    ...
    auth = kakao.auth(code)
    userinfo = kakao.userinfo(auth['access_token'])
    user = {
        "id": str(userinfo['id']),
        "auth_type": "kakao",
    ...
    response.set_cookie(key="kakao_jwt", value=jwt)
...
async def refresh(refresh_token: str):
    ...
    """
    kakao = KakaoOauth(config)
    return kakao.refresh(refresh_token)
...
async def userinfo(access_token: str):
    ...
    """
    kakao = KakaoOauth(config)
    return kakao.userinfo(access_token)
